...
MAPI_CheckData <- function(samples, metric, isMatrix=all(("matrix" %in% class(metric)), (nrow(metric)==ncol(metric)))) {
    ...
    if (isMatrix) {
        if (!("matrix" %in% class(metric))) {stop("'isMatrix' is true, so 'metric' parameter should be a matrix class with colnames and rownames")}
    ...
    tri <- as.vector(my.samples[ (my.samples$ind %notin% my.metric$ind1) & (my.samples$ind %notin% my.metric$ind2) ,  "ind"])
